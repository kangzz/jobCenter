...
// 实例化
    uploader = WebUploader.create({
        ...
        paste: '#uploader',
        swf: data.baseUrl+'/js/plugins/webuploader/Uploader.swf',
        chunked: false,
        chunkSize: 512 * 1024,
    ...
    uploader.onError = function (code) {
        switch (code){
            case "Q_EXCEED_NUM_LIMIT": toastr.warning("最多上传"+data.fileNumLimit+"张图片"); break;
            case "Q_EXCEED_SIZE_LIMIT": toastr.warning("文件大小超过限制"); break;
            case "Q_TYPE_DENIED": toastr.warning("文件类型不支持"); break;
            case "F_DUPLICATE": toastr.warning("文件已选择"); break;
            default :toastr.warning("文件上传错误，错误代码："+code);
        }
    };
    ...
    return uploader;//返回uploader 可以在调用处做出一些操作
}